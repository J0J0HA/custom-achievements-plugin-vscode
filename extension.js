...
function connect() {
  ...
  casws.set_event_handler("new_achievement", async (data) => {
    let result = await vscode.window.showInformationMessage(
        `Achievement unlocked: ${data.name} ${data.level}`, "Show Details",
        "Dismiss");
...
// 10px"><tr><td><img src="${d.image_url}" style="border-radius: 50%; height:
// 150px; width: 150px;"></td><td style="padding: 0px 30px; display: block; top:
// 0px; position:  absolute;"><h1>${d.name}
// ${d.level}</h1><p>${d.description}</p><a href="http${config.secure ? "s" :
// ""}://${config.server}/user/${config.user}">Show all my
// achievements</a></td>`;
// 			}
// 		}
...
// 		_connect(data => {
// 			data.ws.on("message", _setup_events(data.ws,
// data.config)); 			resolve(data.ws);
// 		}, async (data, reason) => {
...
let intervals =
    ...
    async function activate(context) {
      // context.subscriptions.push(vscode.commands.registerCommand("custom-achievements.reconnect",
      // function () { 	setup_websocket().then((nws) => { 		ws = nws
      ...
      context.subscriptions.push(vscode.commands.registerCommand(
          "custom-achievements.reconnect", function() { casws.reconnect(); }));
      ...
      context.subscriptions.push(
          vscode.workspace.onDidChangeTextDocument((changeEvent) => {
            ...
            if (changeEvent.contentChanges[0].text.length == 0) {
              casws.report("text.delete.abs." + changeEvent.document.languageId,
                           changeEvent.contentChanges[0].rangeLength);
              casws.report(
                  "text.delete.past-1." + changeEvent.document.languageId, 1);
              if (changeEvent.contentChanges[0].rangeLength >= 100) {
                casws.report("text.delete.past-100." +
                                 changeEvent.document.languageId,
                             1);
              }
            }
            if (changeEvent.contentChanges[0].text.length > 1) {
              casws.report("text.paste.abs." + changeEvent.document.languageId,
                           changeEvent.contentChanges[0].text.length);
              casws.report(
                  "text.paste.past-1." + changeEvent.document.languageId, 1);
              if (changeEvent.contentChanges[0].text.length >= 100) {
                casws.report("text.paste.past-100." +
                                 changeEvent.document.languageId,
                             1);
              }
            }
      ...
      context.subscriptions.push(
          vscode.workspace.onDidCreateFiles((createEvent) => {
            casws.report("file.create." + createEvent.files[0].languageId,
                         createEvent.files.length);
          }));

      context.subscriptions.push(
          vscode.workspace.onDidDeleteFiles((deleteEvent) => {
            casws.report("file.delete." + deleteEvent.files[0].languageId,
                         deleteEvent.files.length);
          }));

      context.subscriptions.push(
          vscode.workspace.onDidRenameFiles((renameEvent) => {
            casws.report("file.rename." + renameEvent.files[0].languageId,
      ...
      context.subscriptions.push(
          vscode.workspace.onDidChangeConfiguration((event) => {
            ...
            if (event.affectsConfiguration('workbench.colorTheme')) {
              const newTheme = vscode.workspace.getConfiguration().get(
      ...
      intervals.push(setInterval(
          () => { casws.report("vscode.ran-for-full.minute"); }, 60 * 1000));

      intervals.push(setInterval(
          () => { casws.report("vscode.ran-for-full.hour"); }, 60 * 60 * 1000));

      intervals.push(
          setInterval(() => { casws.report("vscode.ran-for-full.day"); },
                      24 * 60 * 60 * 1000));

      intervals.push(
          setInterval(() => { casws.report("vscode.ran-for-full.week"); },
                      7 * 24 * 60 * 60 * 1000));

      casws.set_event_handler("ws-reject", async (error) => {
        let result = await vscode.window.showErrorMessage(
            "Custom Achievements: Connection rejected.", "Retry", "Ignore");
        if (result == "Retry")
          casws.reconnect();
      });
      ...
      casws.set_event_handler("close", async (error) => {
        let result = await vscode.window.showErrorMessage(
            "Lost connection. Reconnect?", "Yes", "No");
        if (result == "Yes")
          casws.reconnect();
      });
